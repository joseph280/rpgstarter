...
namespace Celestia.EditorTools
{
    ...
    public static class W2_WeaponBuilder
    {
        ...
        private const string PATH_AXE          = DIR_WEAPONS  + "/Weapon_Axe.asset";
        private const string PATH_MACE         = DIR_WEAPONS  + "/Weapon_Mace.asset";
        private const string PATH_SPEAR        = DIR_WEAPONS  + "/Weapon_Spear.asset";
        ...
        // Demo branch: primitive stand-ins replace the licensed Blink weapon pack so
        // the project is redistributable. Generated by Demo_PrimitiveAssets — run
        // "Celestia → Demo → 1. Build Primitive Source Assets" before this builder.
        private const string PREFAB_BOW    = Demo_PrimitiveAssets.SRC_BOW;
        private const string PREFAB_AXE    = Demo_PrimitiveAssets.SRC_AXE1H;
        private const string PREFAB_MACE   = Demo_PrimitiveAssets.SRC_MACE;
        private const string PREFAB_SPEAR  = Demo_PrimitiveAssets.SRC_SPEAR;

        // Sword + shield are in _Project/Art/Weapons (not Blink). Wrapper prefabs are
        // generated below with URP/Lit materials bound to the loose albedo PNGs that ship
        // alongside the FBXs.
        ...
        // Bow keeps its existing (already-tuned-by-artist) transform on the LeftHand.
        ...
        [MenuItem("Celestia/W2/5 - Build Weapon Roster (SOs + abilities)")]
        public static void Build()
        {
            ...
            if (shotAbility == null)
                Debug.LogWarning("[W2_WeaponBuilder] BasicShot ability not found — Bow weapon will reference null. Run Celestia/W2/1 first.");

            // Animation clips. Bow uses the recoil clip; melee weapons share the slash.
            ...
            if (slashClip == null) Debug.LogError("[W2_WeaponBuilder] Slash clip missing — run Celestia/W2/2 first.");
            if (bowClip   == null) Debug.LogError("[W2_WeaponBuilder] Bow clip missing — run Celestia/W2/2 first.");
            ...
            var macePrefab  = AssetDatabase.LoadAssetAtPath<GameObject>(PREFAB_MACE);
            var spearPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(PREFAB_SPEAR);

            EnsureWeapon(PATH_BOW,   "Bow",   bowPrefab,   HumanBodyBones.LeftHand,
                         LEFTHAND_BOW_POS, LEFTHAND_BOW_EULER, LEFTHAND_BOW_SCALE,
                         bowClip,   shotAbility);
            EnsureWeapon(PATH_AXE,   "Axe",   axePrefab,   HumanBodyBones.RightHand,
                         RIGHTHAND_POS, RIGHTHAND_EULER, RIGHTHAND_SCALE,
                         slashClip, slashAbility);
            EnsureWeapon(PATH_MACE,  "Mace",  macePrefab,  HumanBodyBones.RightHand,
                         RIGHTHAND_POS, RIGHTHAND_EULER, RIGHTHAND_SCALE,
                         slashClip, slashAbility);
            EnsureWeapon(PATH_SPEAR, "Spear", spearPrefab, HumanBodyBones.RightHand,
                         RIGHTHAND_POS, RIGHTHAND_EULER, RIGHTHAND_SCALE,
                         slashClip, slashAbility);
